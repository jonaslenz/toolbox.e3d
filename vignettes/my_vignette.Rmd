...
# skinfactors are set for each entry separately
  ...
  write.csv(soils,file.path(path,"soil/soil_params.csv"), row.names = FALSE, quote = FALSE)
```
...
# write files to parent folder
  write.csv(soils,file.path(path,"soil_params.csv"), row.names = FALSE, quote = FALSE)
  toolbox.e3d::write.landuse.E3D(POLY_ID = soils$POLY_ID,length = 10, path = file.path(path), filename = "landuse.asc")
